...
@Component({
  selector: 'app-rate-per-item',
  templateUrl: './rate-per-item.component.html',
  styleUrls: ['./rate-per-item.component.scss']
})
export class RatePerItemComponent implements OnInit {
  ...
  loadCharts(){
    this.zone.runOutsideAngular(() => {
      let chart = am4core.create("rateperitem_chart", am4charts.XYChart);
      chart.colors.list = [
      ...
      this.superadminDB.getUtilizationRatePerItem(filterData).subscribe((result: any)=>{
