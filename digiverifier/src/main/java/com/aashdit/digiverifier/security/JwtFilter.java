...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (httpServletRequest.getRequestURI().contains("/api/login/authenticate") || httpServletRequest.getRequestURI().contains("/configuration/ui") || httpServletRequest.getRequestURI().contains("/swagger") || httpServletRequest.getRequestURI().contains("/webjars") || httpServletRequest.getRequestURI().contains("/v2") || httpServletRequest.getRequestURI().contains("/api/allowAll")) {
            skipThis = true;
                            ...
                            if (user != null) {
                                ...
                                try {
                                    if (jwtUtil.validateToken(token, user)) {
                                        ...
                                        final LoggedInUser userDetails = new LoggedInUser(user.getUserName(), user.getPassword(), true, true, true, true, grantedAuthorities, user.getRole(), user);

                                        final Authentication auth = new UsernamePasswordAuthenticationToken(userDetails, null, grantedAuthorities);
